...
void writeGroupsInFile(GroupList groupList, char *fileName) {
  ...
  char *json = jsonifyGroups(groupList);
  fprintf(file, "%s", json);
}
...
GroupList unJsonifyGroupsFromFile(char *fileName) {
  ...
  if (file == NULL) {
    logAction("Error abriendo el archivo", "sistema", 'f');
    return groupList;
  }
  char line[1000];
  while (fgets(line, 1000, file) != NULL) {
    ...
    while (token != NULL) {
      ...
      group->name = token;
      token = strtok(NULL, ",");
      group->maxUsers = atoi(token);
      token = strtok(NULL, ",");
      group->numUsers = atoi(token);
      token = strtok(NULL, ",");
      UserList userList = unJsonifyUserList(token);
      for (int i = 0; i < userList.numUsers; i++) {
        group->users[i] = userList.userList[i];
      }
      token = strtok(NULL, ",");
      ActivityList activityList = unJsonifyActivities(token);
      for (int i = 0; i < activityList.numActivities; i++) {
...
char *jsonifyGroups(GroupList groupList) {
  ...
  for (int i = 0; i < groupList.numGroups; i++) {
    ...
    strcat(json, "\",");
    strcat(json, "\"maxUsers\": ");
    char maxUsers[10];
    sprintf(maxUsers, "%i", groupList.groups[i]->maxUsers);
    strcat(json, maxUsers);
    strcat(json, ",");
    strcat(json, "\"numUsers\": ");
    char numUsers[10];
    sprintf(numUsers, "%i", groupList.groups[i]->numUsers);
    strcat(json, numUsers);
    strcat(json, ",");

    strcat(json, "\"users\": ");
    for (int j = 0; j < groupList.groups[i]->numUsers; j++) {
      addUserToList(&userList, groupList.groups[i]->users[j]);
    }
    ...
    for (int j = 0; j < groupList.groups[i]->numActivities; j++) {
      addActivityToList(activityList, groupList.groups[i]->activityList[j]);
    }
...
char *processGroupsFile(char *fileName) {
  GroupList groupList = unJsonifyGroupsFromFile(fileName);
  char *json = jsonifyGroups(groupList);
  return json;
}
